...
particular an "AXSM" 6-coil sensor with an optional conducting shield can),
and for inverse-solving the currents needed to hit a target field.

一个平面线圈阵列的 2D 磁场仿真小工具包（重点是一个 6 线圈的 "AXSM" 传感器，外加可选的导体屏蔽层），
并支持反解出能实现目标磁场分布的电流。
...
- **`AXSM_coils.py`** — `AXSMCoils(Coils)`: the 6-coil AXSM sensor ring.
...
- **`current_solver.py`** — `CurrentSolver`: given a set of **sample
  ...
  **current points** (positions with an unknown current), builds the
  ...
  (e.g. `AXSMCoils`), so you solve for physical DOFs instead of raw
...
- **`example.py`** — end-to-end demo: solves jointly for the 6 AXSM coil
...
axsm.plot(path="fields.png")          # field-line plot with I1..I6 legend
print(axsm.B_field(0, 0))             # (Bx, By) in Tesla at the origin
```

```python
from current_solver import CurrentSolver

solver = CurrentSolver.from_current_source(axsm)
solver.add_sample_point(x=1, y=0, Bx=0.0, By=1e-3)   # target field at (1, 0) mm
I_free = solver.solve()                               # solve for I1..I6
solved = AXSMCoils(currents=I_free, radius=axsm.radius, coil_length=axsm.coil_length)
...
- `AXSMCoils`'s exact mapping from `I1..I6` labels / winding sign
...
- **`AXSM_coils.py`** —— `AXSMCoils(Coils)`：6 线圈的 AXSM 传感器环。6 个物理线圈中的每一个
...
- **`current_solver.py`** —— `CurrentSolver`：给定一组**采样点**（指定位置上期望的目标
  ...
  方法的 `Coils` 类对象（比如 `AXSMCoils`）构造求解器，这样解出的就是物理自由度而不是逐点电流。
...
- **`example.py`** —— 完整示例：联合求解 6 个 AXSM 线圈电流和 100 个屏蔽层电流（共 106 个
...
axsm = AXSMCoils(currents=[729.3, 1000, 270.7, -729.3, -1000, -270.7])
axsm.plot(path="fields.png")          # 画磁场线图，带 I1..I6 图例
print(axsm.B_field(0, 0))             # 原点处的 (Bx, By)，单位 T
```
...
- `AXSMCoils` 里 `I1..I6` 标签、绕线正负号约定与实物参考图的精确对应关系，是文档中说明过的
