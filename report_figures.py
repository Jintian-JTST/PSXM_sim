...
def latex_table(qt, dt, fname="table_scan.tex"):
    """The shield-radius table rows, one macro per mode.

    Nine columns across one page is too wide to read, so the two modes are
    emitted separately (\\scantablequad and \\scantabledip) and main.tex
    sets them as two tables.  Emitted as macro definitions rather than raw
    rows because \\input inside a tabular breaks the alignment, while a
    macro expanding to the rows does not.
    """
    import os
    ...
    def rows(tab, field_key, fmt):
        out = []
        for i in range(len(tab["R"])):
            out.append("    %.3f & %s & %.0f & %s & %s \\\\" % (
                tab["R"][i], fmt % tab[field_key][i],
                sheet(tab["Ish"][i], tab["R"][i]),
                sci(tab["leaknear"][i]), sci(tab["leakfar"][i])))
        return out

    p = os.path.join(REPORT_DIR, fname)
    with open(p, "w", encoding="utf-8") as f:
        f.write("% auto-generated by report_figures.py -- do not edit\n")
        f.write("\\def\\scantablequad{%\n")
        f.write("\n".join(rows(qt, "G", "%.3f")) + "%\n}\n")
        f.write("\\def\\scantabledip{%\n")
        f.write("\n".join(rows(dt, "B0", "%.2f")) + "%\n}\n")
    print("wrote", p)
